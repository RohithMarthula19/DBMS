...
export default function Reversals() {
  ...
  const statusColor = s => ({ PENDING:'#d97706', APPROVED:'#16a34a', REJECTED:'#dc2626', EXPIRED:'#4b5563' }[s] || '#1f2937');

  return (
    <div style={{ fontFamily:'sans-serif', background:'#f8fafc', minHeight:'100vh' }}>
      <Navbar />
      <div style={{ padding:24, maxWidth:900, margin:'0 auto' }}>
        <h2 style={{ color:'#000000' }}>↩️ Transaction Reversals</h2>

        <div style={card}>
          <h3 style={{ margin:'0 0 16px', color:'#000000' }}>Pending Requests (as Receiver)</h3>
          {!pending.length && <p style={{ color:'#000000' }}>No pending reversal requests on your accounts</p>}
          {pending.map(r => (
            <div key={r.request_id} style={{ border:'1px solid #fde68a', background:'#fffbeb', borderRadius:8, padding:16, marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between' }}>
                <strong style={{ color:'#000000' }}>₹{parseFloat(r.amount).toFixed(2)}</strong>
                <span style={{ fontSize:12, color:'#000000', fontWeight:600 }}>Expires: {new Date(r.expires_at).toLocaleString()}</span>
              </div>
              <p style={{ margin:'6px 0', fontSize:13, color:'#000000', fontWeight:500 }}>{r.description}</p>
              <p style={{ margin:'4px 0', fontSize:12, color:'#000000', fontWeight:600 }}>Reason: {r.reason}</p>
              <div style={{ display:'flex', gap:8, marginTop:10 }}>
                ...
                <button onClick={() => resolve(r.request_id, 'REJECTED')}
                  style={{ padding:'8px 16px', background:'#dc2626', color:'white', border:'none', borderRadius:6, cursor:'pointer', fontWeight:'bold' }}>
                  ❌ Reject
                </button>
              </div>
            </div>
        ...
        <div style={{ ...card, marginTop:20 }}>
          <h3 style={{ margin:'0 0 16px', color:'#000000' }}>Reversal History</h3>
          {history.map(r => (
            <div key={r.request_id} style={{ display:'flex', justifyContent:'space-between', padding:'10px 0', borderBottom:'1px solid #f3f4f6', fontSize:13 }}>
              <span style={{ color:'#000000' }}>{r.description} — ₹{parseFloat(r.amount).toFixed(2)}</span>
              <span style={{ color: statusColor(r.status), fontWeight:'bold' }}>{r.status}</span>
            </div>
          ))}
          {!history.length && <p style={{ color:'#000000' }}>No reversal history</p>}
        </div>
      </div>
    </div>
  );
