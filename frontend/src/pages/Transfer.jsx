...
export default function Transfer() {
  ...
  const doTransfer = async () => {
    try {
      const res = await client.post('/transactions/transfer', form, {
        headers: { 'idempotency-key': uuidv4() }
      });
      setResult({ ok: true, msg: `✅ Transfer successful! ID: ${res.data.transaction.transaction_id}` });
    } catch (e) { setResult({ ok: false, msg: e.response?.data?.error }); }
  };

  const doDeposit = async () => {
    try {
      const res = await client.post('/transactions/deposit', depositForm);
      setResult({ ok: true, msg: `✅ Deposit successful! ID: ${res.data.transaction.transaction_id}` });
    } catch (e) { setResult({ ok: false, msg: e.response?.data?.error }); }
  };
  ...
  return (
    <div style={{ fontFamily:'sans-serif', background:'#f8fafc', minHeight:'100vh' }}>
      ...
      <div style={{ padding:24, maxWidth:900, margin:'0 auto' }}>
        <h2 style={{ color:'#000000' }}>Transfers & Deposits</h2>
        {result && (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }}>
          {/* Transfer */}
          <div style={card}>
            <h3 style={{ color:'#000000', margin:'0 0 16px' }}>💸 Send Money</h3>
          ...
          {/* Deposit */}
          <div style={card}>
            <h3 style={{ color:'#000000', margin:'0 0 16px' }}>💰 Deposit Funds</h3>
...
const btn = { padding:'10px 20px', background:'#000000', color:'white', border:'none', borderRadius:6, cursor:'pointer', fontSize:14, width:'100%', marginTop:4, fontWeight:'bold' };