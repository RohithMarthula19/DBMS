...
export default function Scheduled() {
  ...
  const statusColor = s => ({ ACTIVE:'#16a34a', COMPLETED:'#2563eb', CANCELLED:'#dc2626' }[s] || '#1f2937');

  return (
    <div style={{ fontFamily:'sans-serif', background:'#f8fafc', minHeight:'100vh' }}>
      <Navbar />
      <div style={{ padding:24, maxWidth:1000, margin:'0 auto' }}>
        <h2 style={{ color:'#000000' }}>⏰ Scheduled Transactions</h2>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1.5fr', gap:24 }}>
          <div style={card}>
            <h3 style={{ margin:'0 0 16px', color:'#000000' }}>Create Schedule</h3>
              ...
              <div key={s.schedule_id} style={{ border:'1px solid #e5e7eb', borderRadius:8, padding:12, marginBottom:10 }}>
                ...
                {s.status === 'ACTIVE' && (
                  <button onClick={() => cancel(s.schedule_id)}
                    style={{ marginTop:8, padding:'6px 12px', fontSize:11, background:'#000000', color:'white', border:'none', borderRadius:4, cursor:'pointer', fontWeight:'bold' }}>
                    Cancel
                  </button>
                )}
              </div>
            ))}
            {!schedules.length && <p style={{ color:'#000000' }}>No scheduled payments</p>}
          </div>
        </div>
      </div>
    </div>
