...
export default function Fraud() {
  ...
  const sevColor = s => ({ HIGH:'#dc2626', MEDIUM:'#d97706', LOW:'#16a34a' }[s] || '#1f2937');

  return (
    <div style={{ fontFamily:'sans-serif', background:'#f8fafc', minHeight:'100vh' }}>
      <Navbar />
      <div style={{ padding:24, maxWidth:1000, margin:'0 auto' }}>
        <h2 style={{ color:'#000000' }}>🚨 Fraud Detection Dashboard</h2>

        {summary.length > 0 && (
          <div style={{ display:'flex', gap:16, marginBottom:24 }}>
            {summary.map(s => (
              <div key={s.account_id} style={{ ...card, flex:1 }}>
                <div style={{ fontSize:12, color:'#000000', fontWeight:700 }}>Account Summary</div>
                <div style={{ fontSize:24, fontWeight:'bold', color: s.high_severity > 0 ? '#dc2626' : '#16a34a' }}>
                  {s.total_flags} flags
                </div>
                <div style={{ fontSize:12, color:'#000000' }}>{s.high_severity} HIGH · {s.open_flags} open</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={card}>
          <h3 style={{ margin:'0 0 16px', color:'#000000' }}>Fraud Flags</h3>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#f1f5f9' }}>
                {['Rule Triggered','Severity','Transaction','Detected At','Status','Action'].map(h => (
                  <th key={h} style={{ padding:'10px 12px', textAlign:'left', fontWeight:700, color:'#000000' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {flags.map(f => (
                <tr key={f.flag_id} style={{ borderBottom:'1px solid #f3f4f6', background: f.status === 'OPEN' ? '#fff7ed' : 'white' }}>
                  ...
                  <td style={{ padding:'10px 12px', fontSize:11, color:'#000000' }}>
                    {f.amount ? `₹${parseFloat(f.amount).toFixed(2)}` : '—'}
                  </td>
                  <td style={{ padding:'10px 12px', fontSize:11, color:'#000000' }}>{new Date(f.created_at).toLocaleString()}</td>
                  <td style={{ padding:'10px 12px' }}>
                  ...
                  <td style={{ padding:'10px 12px' }}>
                    {f.status === 'OPEN' && (
                      <button onClick={() => resolve(f.flag_id)}
                        style={{ padding:'4px 8px', fontSize:11, background:'#000000', color:'white', border:'none', borderRadius:4, cursor:'pointer', fontWeight:'bold' }}>
                        Resolve
                      </button>
                    )}
                  </td>
              ...
              {!flags.length && <tr><td colSpan={6} style={{ padding:24, textAlign:'center', color:'#000000' }}>No fraud flags detected</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
